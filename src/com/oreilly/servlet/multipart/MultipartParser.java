...
/**
 * A utility class to handle <code>multipart/form-data</code> requests, the kind of
 * requests that support file uploads. This class uses a "pull" model where the reading of
 * incoming files and parameters is controlled by the client code, which allows incoming
 * files to be stored into any <code>OutputStream</code>. If you wish to use an API
 * which resembles <code>HttpServletRequest</code>, use the "push" model
 * <code>MultipartRequest</code> instead. It's an easy-to-use wrapper around this class.
 * <p>
 * This class can receive arbitrarily large files (up to an artificial limit you can set),
 * and fairly efficiently too. It cannot handle nested data (multipart content within
 * multipart content). It <b>can</b> now with the latest release handle internationalized
 * content (such as non Latin-1 filenames).
 * <p>
 * It also optionally includes enhanced buffering and Content-Length limitation. Buffering
 * is only required if your servlet container is poorly implemented (many are, including
 * Tomcat 3.2), but it is generally recommended because it will make a slow servlet
 * container a lot faster, and will only make a fast servlet container a little slower.
 * Content-Length limiting is usually only required if you find that your servlet is
 * hanging trying to read the input stram from the POST, and it is similarly recommended
 * because it only has a minimal impact on performance.
 * <p>
 ...
 * @version 1.11, 2002/11/01, added constructor that takes an encoding, to make sure chars
 * are always read correctly
 * @version 1.10, 2002/11/01, added support for a preamble before the first boundary
 * marker
...
public class MultipartParser
{
	...
	/**
	 * Creates a <code>MultipartParser</code> from the specified request, which limits
	 * the upload size to the specified length, buffers for performance and prevent
	 * attempts to read past the amount specified by the Content-Length.
	 * 
	...
	/**
	 * Creates a <code>MultipartParser</code> from the specified request, which limits
	 * the upload size to the specified length, and optionally buffers for performance and
	 * prevents attempts to read past the amount specified by the Content-Length.
	 * 
	 * @param req the servlet request.
	 * @param maxSize the maximum size of the POST content.
	 * @param buffer whether to do internal buffering or let the server buffer, useful for
	 * servers that don't buffer
	 * @param limitLength boolean flag to indicate if we need to filter the request's
	 * input stream to prevent trying to read past the end of the stream.
	 */
	 ...
	 * Creates a <code>MultipartParser</code> from the specified request, which limits
	 * the upload size to the specified length, and optionally buffers for performance and
	 * prevents attempts to read past the amount specified by the Content-Length, and with
			...
			boolean limitLength, String encoding) throws IOException
	{
		...
		if (length > maxSize)
		{
			throw new IOException("Posted content length of " + length + " exceeds limit of "
					+ maxSize);
		}
	...
	/**
	 * Sets the encoding used to parse from here onward. The default is ISO-8859-1.
	 * Encodings are actually best passed into the contructor, so even the initial line
	 * reads are correct.
	 * 
	...
	/**
	 * Read the next part arriving in the stream. Will be either a <code>FilePart</code>
	 * or a <code>ParamPart</code>, or <code>null</code> to indicate there are no
	 * more parts to read. The order of arrival corresponds to the order of the form
	 * elements in the submitted form.
	 * 
	 ...
	 * 
	 * @return a String containing the next line of input from the stream, or null to
