 ...
 * providing access to a single MIME part contained with in which ends with the boundary
 * specified. It uses buffering to provide maximum performance.
 * <p>
 * Note the <code>readLine</code> method of <code>ServletInputStream</code> has the
 * annoying habit of adding a \r\n to the end of the last line. Since we want a
 * byte-for-byte transfer, we have to cut those chars. This means that we must always
 * maintain at least 2 characters in our buffer to allow us to trim when necessary.
 * 
 * @author Geoff Soutter
 * @author Jason Hunter
 * @version 1.4, 2002/11/01, fix for "unexpected end of part" caused by boundary newlines
...
public class PartInputStream extends FilterInputStream
{
	...
	/**
	 * Creates a <code>PartInputStream</code> which stops at the specified boundary from
	 * a <code>ServletInputStream<code>.
	 * 
	...
	/**
	 * Fill up our buffer from the underlying input stream, and check for the boundary
	 * that signifies end-of-file. Users of this method must ensure that they leave
	 * exactly 2 characters in the buffer before calling this method (except the first
	 * time), so that we may only use these characters if a boundary is not found in the
	 * first line read.
	 * 
	 ...
	 * <p>
	 * Returns <code>-1</code> (end of file) when the MIME boundary of this part is
	 * encountered.
	 * 
	 ...
	 * buffering gracefully, and is not same as the length of the part arriving in this
	 * stream.
	 * 
	 ...
	 * stream.
	 * <p>
	 * This method will read any unread data in the MIME part so that the next part starts
	 * an an expected place in the parent <code>InputStream</code>. Note that if the
	 * client code forgets to call this method on error, <code>MultipartParser</code>
	 * will call it automatically if you call <code>readNextPart()</code>.
	 * 
	 * @exception IOException if an I/O error occurs.
	 */
	...
	{
		if(in instanceof LimitedServletInputStream)
		{
			LimitedServletInputStream lin = (LimitedServletInputStream)in;
			return lin.getTotalExpected();
		}
		return 0;
	}
}
